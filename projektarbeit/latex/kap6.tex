...
Lokalisierung und Kartierung wurden über den Absolute Trajectory Error, kurz ATE, und die Wiederholbarkeit der erzeugten Karte bewertet. In Testfall T3.1 (Loop-Closure-Kartenqualität) betrug der ATE als RMSE $0{,}190\,\mathrm{m}$ und als MAE $0{,}161\,\mathrm{m}$. In Testfall T3.2 (dedizierte ATE-Messung mit bestehender Karte) sank der ATE (RMSE) auf $0{,}030\,\mathrm{m}$ (Messprotokoll Phase~3). Das Akzeptanzkriterium lag bei $0{,}20\,\mathrm{m}$.

Beide Testfälle unterschreiten das Akzeptanzkriterium. Die Differenz zwischen T3.1 und T3.2 erklärt sich aus der gleichzeitigen Kartenerstellung und Lokalisierung in T3.1 gegenüber der reinen Lokalisierung auf bestehender Karte in T3.2. Für einen kostengünstigen Innenraum-Prototyp ist das Ergebnis ausreichend, um Ziele auf Kartenbasis anzufahren und nach einem Neustart wieder zu lokalisieren. Die Kernanforderung an Lokalisierung und Kartierung gilt damit auf Basis der vorliegenden Messdaten als erfüllt.

\subsection{Zielanfahrt und Bahnverfolgung}

Die Qualität der Zielanfahrt wurde über Punkt-zu-Punkt-Fahrten bewertet (Messprotokoll Phase 4, T4.1). Die mittlere Positionsabweichung betrug $1{,}01\,\mathrm{cm}$, die mittlere Winkelabweichung $1{,}94^\circ$.

Diese Werte zeigen eine für die Innenraumanwendung hohe Zielgenauigkeit. Die Abweichungen liegen deutlich unter den Akzeptanzkriterien ($< 10\,\mathrm{cm}$ bzw.\ $< 8{,}6^\circ$) und belegen eine zuverlässige Navigation zwischen definierten Zielpunkten. Die Zielanfahrt bestätigt zugleich, dass die zuvor kalibrierte Odometrie, die IMU-Korrektur und die globale Kartenreferenz im Betrieb zusammenwirken.
...
Der Raspberry Pi 5 blieb im regulären Betrieb unter $80\,\%$ CPU-Last. Der Docker-Container für ROS-2-Integration und MJPEG-Stream lag im Mittel bei etwa $35\,\%$ Auslastung. Der hardwarebeschleunigte Vision-Prozess \texttt{host\_hailo\_runner.py} lief nativ auf dem Host und band den PCIe-Beschleuniger ohne erhebliche Zusatzlast für die CPU ein. Auf Mikrocontroller-Ebene beanspruchte die Firmware des Drive-Knotens auf Core 1 weniger als $40\,\%$ der verfügbaren Rechenkapazität. Der Regulated Pure Pursuit Controller auf dem Raspberry Pi erreichte Rechenraten von mehr als $2{.}000\,\mathrm{Hz}$ bei einem geforderten Minimum von $20\,\mathrm{Hz}$ (\texttt{controller\_frequency} in \texttt{nav2\_params.yaml}).
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l L l L}
    ...
    \addlinespace
    F03 & Lokalisierung und Kartierung für den Innenraum & erfüllt & ATE (RMSE): T3.1 $0{,}190\,\mathrm{m}$ / T3.2 $0{,}030\,\mathrm{m}$; ATE (MAE): T3.1 $0{,}161\,\mathrm{m}$. Beide unterschreiten das Kriterium von $0{,}20\,\mathrm{m}$. \\
    \addlinespace
    ...
    \addlinespace
    F05 & Bedien- und Leitstandsebene als Betriebswerkzeug & erfüllt & Testfälle T5.1--T5.5 (Messprotokoll Phase~5) belegen cmd\_vel-Latenz, Telemetrie-Vollständigkeit, Deadman-Timer, Audio-Feedback und Notaus. Alle 5/5 Testfälle bestanden. \\
    \addlinespace
...
Die Sensor- und Sicherheitsbasis ist durch die Einzelbewertung von Ultraschall, Cliff-Sensor, IMU und Batterieüberwachung vollständig belegt. Die End-to-End-Latenz der Cliff-Safety-Kette von $2{,}0\,\mathrm{ms}$ und der Bremsweg von $1{,}0\,\mathrm{cm}$ bei $0{,}2\,\mathrm{m/s}$ bestätigen die Schutzfunktion unter realistischen Betriebsbedingungen. Die Bedien- und Leitstandsebene ist mit allen fünf Testfällen (T5.1--T5.5) vollständig validiert. Die Sprachschnittstelle bleibt eine vorbereitete Erweiterung und gehört folgerichtig nicht zum belegten Kernumfang.
