...
Der Fahrkern muss Bewegungsbefehle mit konstantem Zeitverhalten umsetzen. Gemessen wurde deshalb der Regelzyklus der PID-Schleife auf dem Drive-Knoten (Messprotokoll Phase 1, T1.1/T1.2). Die Schleife hielt einen Takt von $50\,\mathrm{Hz}$ bei einem Jitter von unter $2\,\mathrm{ms}$ ein. Der Datenverlust der micro-ROS-Kommunikation lag unter $0{,}1\,\%$.
...
Die Sensor- und Sicherheitsbasis umfasst vier Sensorgruppen: Ultraschall, Cliff-Sensor, IMU und Batteriemonitoring. Jede Gruppe wurde einzeln gegen ihre Akzeptanzkriterien geprueft. Die Messungen erfolgten am 09.03.2026 mit den Validierungsskripten `sensor_test.py` (Ultraschall und Cliff-Sensor, 7 Testfaelle) und `imu_test.py` (IMU-Drift, Beschleunigungsmesser-Bias, Heading-Vergleich, 4 Testfaelle).
...
Lokalisierung und Kartierung wurden ueber den Absolute Trajectory Error, kurz ATE, und die Wiederholbarkeit der erzeugten Karte bewertet. In Testfall T3.1 (Loop-Closure-Kartenqualitaet) betrug der ATE als RMSE $0{,}190\,\mathrm{m}$ und als MAE $0{,}161\,\mathrm{m}$. In Testfall T3.2 (dedizierte ATE-Messung mit bestehender Karte) sank der ATE (RMSE) auf $0{,}030\,\mathrm{m}$ (Messprotokoll Phase 3). Das Akzeptanzkriterium lag bei $0{,}20\,\mathrm{m}$.

Beide Testfaelle unterschreiten das Akzeptanzkriterium. Die Differenz zwischen T3.1 und T3.2 erklaert sich aus der gleichzeitigen Kartenerstellung und Lokalisierung in T3.1 gegenueber der reinen Lokalisierung auf bestehender Karte in T3.2. Fuer einen kostenguenstigen Innenraum-Prototyp ist das Ergebnis ausreichend, um Ziele auf Kartenbasis anzufahren und nach einem Neustart wieder zu lokalisieren. Die Kernanforderung an Lokalisierung und Kartierung gilt damit auf Basis der vorliegenden Messdaten als erfuellt.

### 6.3.2 Zielanfahrt und Bahnverfolgung

Die Qualitaet der Zielanfahrt wurde ueber Punkt-zu-Punkt-Fahrten bewertet (Messprotokoll Phase 4, T4.1). Die mittlere Positionsabweichung betrug $1{,}01\,\mathrm{cm}$, die mittlere Winkelabweichung $1{,}94^\circ$.

Diese Werte zeigen eine fuer die Innenraumanwendung hohe Zielgenauigkeit. Die Abweichungen liegen deutlich unter den Akzeptanzkriterien ($< 10\,\mathrm{cm}$ bzw. $< 8{,}6^\circ$) und belegen eine zuverlaessige Navigation zwischen definierten Zielpunkten. Die Zielanfahrt bestaetigt zugleich, dass die zuvor kalibrierte Odometrie, die IMU-Korrektur und die globale Kartenreferenz im Betrieb zusammenwirken.
...
Der Raspberry Pi 5 blieb im regulaeren Betrieb unter $80\,\%$ CPU-Last. Der Docker-Container fuer ROS-2-Integration und MJPEG-Stream lag im Mittel bei etwa $35\,\%$ Auslastung. Der hardwarebeschleunigte Vision-Prozess `host_hailo_runner.py` lief nativ auf dem Host und band den PCIe-Beschleuniger ohne erhebliche Zusatzlast fuer die CPU ein. Auf Mikrocontroller-Ebene beanspruchte die Firmware des Drive-Knotens auf Core 1 weniger als $40\,\%$ der verfuegbaren Rechenkapazitaet. Der Regulated Pure Pursuit Controller auf dem Raspberry Pi erreichte Rechenraten von mehr als $2{.}000\,\mathrm{Hz}$ bei einem geforderten Minimum von $20\,\mathrm{Hz}$ (`controller_frequency` in `nav2_params.yaml`).
...
| F03 | Lokalisierung und Kartierung fuer den Innenraum                  | erfuellt           | ATE (RMSE): T3.1 $0{,}190\,\mathrm{m}$ / T3.2 $0{,}030\,\mathrm{m}$; ATE (MAE): T3.1 $0{,}161\,\mathrm{m}$. Beide unterschreiten das Kriterium von $0{,}20\,\mathrm{m}$.                                     |
...
| F05 | Bedien- und Leitstandsebene als Betriebswerkzeug                | erfuellt           | Testfaelle T5.1-T5.5 (Messprotokoll Phase 5) belegen cmd\_vel-Latenz, Telemetrie-Vollstaendigkeit, Deadman-Timer, Audio-Feedback und Notaus. Alle 5/5 Testfaelle bestanden.                                          |
