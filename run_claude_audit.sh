...
run_block() {
  ...
  echo "  [${id}/5] Starte: ${title} ..."
  if claude -p "${prompt} ${FORMAT_HINT}" > "$outfile" 2>"$TMPDIR_AUDIT/err_${id}.log"; then
    echo "  [${id}/5] Fertig:  ${title}"
  else
    echo "  [${id}/5] FEHLER: ${title} (Exit $?)"
    echo "_Fehler bei der Auswertung. Siehe err_${id}.log._" > "$outfile"
  fi
}

echo "Starte parallele Dokumentenpruefung (5 Bloecke) ..."
echo ""

# Block 1-5 parallel starten
...
run_block 5 "Präsentationen und Kolloquium (planung/vortrag/)" \
  "Lese CLAUDE.md und analysiere den finalen Code in amr/. Pruefe alle Präsentationsdateien im Verzeichnis planung/vortrag/ (insbesondere vortrag_showcase.md und vortrag_kolloquium.tex). Kontrolliere, ob die für das Publikum aufbereiteten Kennzahlen (z. B. 50 Hz Regelung, < 2 ms Jitter, 34 ms Edge-Latenz, < 20 ms CAN-Notstopp) und die Architekturbeschreibungen exakt dem aktuellen Implementierungsstand im Code entsprechen. Dokumentiere alle Diskrepanzen, auch wenn sie durch didaktische Vereinfachung entstanden sind." &

# Warten bis alle 5 fertig sind
wait
echo ""
...
# Report zusammenbauen (Reihenfolge garantiert)
{
  ...
  echo "Pruefziele: docs/, planung/, projektarbeit/, planung/vortrag/"
  echo "---"
  echo ""
  ...
  for i in 1 2 3 4 5; do
    case $i in
      ...
      5) echo "## 5. Präsentationen und Kolloquium (planung/vortrag/)" ;;
    esac
    echo ""
