...
class DockingTestNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'Docking-Test bereit. {self.num_versuche} Versuche geplant.')
        self.get_logger().info(
            'Eingabe "s" + Enter im Terminal startet den naechsten Versuch.')
    ...
    def auswertung(self):
        ...
        self.get_logger().info(
            f'Erfolgsquote >= 80%: {"PASS" if pass_erfolg else "FAIL"} '
            f'({erfolgsquote:.0f}%)')
