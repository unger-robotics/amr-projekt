...
class HailoInferenceNode(Node):
    ...
    def __init__(self):
        ...
        # Parameter
        self.declare_parameter("model_path", "hardware/models/yolov8s.hef")
        ...
        self.target_period = (
            1.0 / self.get_parameter("inference_hz").get_parameter_value().double_value
        )
        ...
        if not HAS_HAILO:
            self.get_logger().error(
                "hailo_platform nicht installiert! pip3 install hailort oder HailoRT-SDK pruefen."
            )
            return
    ...
    def _postprocess(self, raw_output: dict, orig_h: int, orig_w: int) -> list:
        ...
        for _name, data in raw_output.items():
            ...
            for i in range(num_predictions):
                ...
                label = (
                    COCO_LABELS[class_id] if class_id < len(COCO_LABELS) else f"class_{class_id}"
                )
    ...
    def _inference_tick(self):
        ...
        input_data = {self.input_vstream_info[0].name: np.expand_dims(preprocessed, axis=0)}
        ...
        if detections:
            ...
            self.get_logger().info(
                f"{len(detections)} Objekt(e) erkannt in {dt_ms:.1f} ms: {', '.join(labels)}"
            )
