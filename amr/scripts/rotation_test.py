...
try:
    ...
    try:
        from my_bot.amr_utils import ANSI_GREEN, ANSI_RED, ANSI_BOLD, ANSI_RESET
    except ImportError:
        ANSI_GREEN = "\033[32m"
        ANSI_RED = "\033[31m"
        ANSI_BOLD = "\033[1m"
...
class RotationController(Node):
    def __init__(self, target_deg=360.0):
        ...
        self.get_logger().info(
            f'Ziel: {target_deg:.1f} Grad Drehung mit IMU-Gyro-Feedback')
    ...
    def print_result(self, elapsed):
        ...
        if passed:
            print(f'  {ANSI_GREEN}{ANSI_BOLD}PASS '
                  f'- Ziel erreicht!{ANSI_RESET}')
        else:
            print(f'  {ANSI_RED}{ANSI_BOLD}FAIL '
                  f'- Timeout{ANSI_RESET}')
        print('=' * 60)
