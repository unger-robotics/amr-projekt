...
| Datei | Beschreibung |
|---|---|
| `odom_to_tf.py` | Bruecke /odom → TF (odom → base_link), Identity-TF bei Start bis erste /odom eintrifft |
...
| `voice_command_node.py` | Sprachsteuerung: Gemini Audio-STT (Cloud, primaer) / faster-whisper (Offline-Fallback), Wake-Word + VAD-Modus, Regex-Intent-Parsing → `/voice/command` + `/voice/text` |
