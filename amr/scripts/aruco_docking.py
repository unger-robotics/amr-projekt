...
class DockingNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Docking-Node gestartet. Suche Marker ID {self.target_marker_id}.")
    ...
    def image_callback(self, msg):
        ...
        if ids is not None and self.target_marker_id in ids.flatten():
            ...
            if marker_width > self.docking_threshold:
                ...
                self.get_logger().info(f"DOCKING COMPLETE. Marker-Breite: {marker_width:.0f} px")
                self.stop_robot()
                return
    ...
    def watchdog_callback(self):
        ...
        if (now - self.start_time) > self.timeout_sec:
            ...
            self.get_logger().warn(f"TIMEOUT nach {self.timeout_sec:.0f} s. Docking abgebrochen.")
            self.stop_robot()
            return
