...
def generate_report(results, output_dir):
    ...
    report.append(
        f"- Orientierungsfehler (yaw): < {YAW_TOLERANCE} rad ({math.degrees(YAW_TOLERANCE):.1f} Grad)"
    )
    report.append("")
    report.append("## Ergebnisse")
    report.append("")
    report.append(
        "| Waypoint | Status | xy-Fehler (m) | yaw-Fehler (rad) | Dauer (s) | Bestanden |"
    )
    report.append(
        "|----------|--------|---------------|-------------------|-----------|-----------|"
    )
    ...
    for r in results:
        ...
        report.append(
            f"| {r['waypoint']} | {r['status']} | {xy} | {yaw} | {r['duration']:.1f} | {status} |"
        )
...
def main():
    if not ROS2_AVAILABLE:
        print(
            "FEHLER: ROS2 (rclpy, nav2_msgs) nicht verfuegbar. "
            "Bitte ROS2 Humble sourcing durchfuehren."
        )
        sys.exit(1)

    parser = argparse.ArgumentParser(description="Automatisierter Navigationstest fuer AMR")
    parser.add_argument(
        "--timeout", type=int, default=60, help="Timeout pro Waypoint in Sekunden (Standard: 60)"
    )
